...
#
setPointAlg <- function(beta_expo,csvdata,sigmaPar,column_select){
  ...
  print(sd(values_raw[[vals[[1]]]]))
  for(i in 1:length(vals)){
    std_dev_df[1,i] <- sd(values_raw[[vals[[i]]]])
  }
  ...
  for(i in 1:length(vals)){
    mean_df[1,i] <- mean(values_raw[[vals[[i]]]])
  }
